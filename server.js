...
app.get('/attempts', async (req, res) => {
    try {
        ...
        console.log(`Fetching attempts: teacherId=${teacherId}, studentId=${studentId}`);
...
app.delete('/attempts/:id', async (req, res) => {
    try {
        const { data: attempt } = await supabase.from('attempts').select('video_path').eq('id', req.params.id).single();
        if (attempt && attempt.video_path) {
            const fp = path.join(__dirname, 'uploads', path.basename(attempt.video_path));
...
app.post('/proctoring/upload', upload.single('video'), (req, res) => {
    try {
        ...
        console.log(`Video received: ${req.file.filename}, Size: ${req.file.size} bytes`);
        ...
        console.error("Error in /proctoring/upload:", e.message);
